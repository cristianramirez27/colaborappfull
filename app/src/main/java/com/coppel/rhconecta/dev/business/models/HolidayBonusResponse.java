package com.coppel.rhconecta.dev.business.models;

import java.io.Serializable;

public class HolidayBonusResponse extends HolidaysBaseResponse {

    private Data data;

    public Data getData() {
        return data;
    }

    public void setData(Data data) {
        this.data = data;
    }

    public class Data {
        private Response response;

        public Response getResponse() {
            return response;
        }

        public void setResponse(Response response) {
            this.response = response;
        }
    }

    public class Response implements Serializable {
        private String fec_primavacacional;
        private String fec_diaprimavacacional;
        private int clv_estado;
        private String des_mensaje;


        public String getFec_primavacacional() {
            return fec_primavacacional;
        }

        public void setFec_primavacacional(String fec_primavacacional) {
            this.fec_primavacacional = fec_primavacacional;
        }

        public String getFec_diaprimavacacional() {
            return fec_diaprimavacacional;
        }

        public void setFec_diaprimavacacional(String fec_diaprimavacacional) {
            this.fec_diaprimavacacional = fec_diaprimavacacional;
        }

        public int getClv_estado() {
            return clv_estado;
        }

        public void setClv_estado(int clv_estado) {
            this.clv_estado = clv_estado;
        }

        public String getDes_mensaje() {
            return des_mensaje;
        }

        public void setDes_mensaje(String des_mensaje) {
            this.des_mensaje = des_mensaje;
        }
    }
}
